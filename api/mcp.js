...
async function callSendEmail(args) {
  const { to, subject, body, cc, confirm, attachments } = args;
  ...
  return { preview: false, text: `✓ Email sent to ${to}` };
}
...
async function handleMcp(rpc) {
  ...
  if (method === 'initialize') return { jsonrpc: '2.0', id, result: { protocolVersion: '2024-11-05', capabilities: { tools: {} }, serverInfo: { name: 'ms365-mailer', version: '1.0.0' } } };
